...
SeuObj.singlet.merge <- readRDS("Outputs/231122_SeuInt3_1st_fil_int_mayt1000lowt60000_lowtInfrb_SN_SoupXOut.rds")
...
```
Delate cluster 20 and 21 and join 19 with 1:

```{r}
SeuInt <- subset(SeuInt, subset = seurat_clusters != "20" & seurat_clusters != "21")
...
SeuInt@meta.data$Slc6a3g <-
  ifelse(GetAssayData(SeuInt, slot = "counts")["Slc6a3", ] > 0, 1, 0) %>% as.factor()
...
plot3 <- DimPlot(SeuInt, group.by = "Slc6a3g", order = T) + th
plot4 <- DimPlot(SeuInt, group.by = "Dbhg", order = T) + th
layout1 <- "
ABC
DFG
HIJ
...
# SeuInt$seurat_clusters <- factor(SeuInt$seurat_clusters, levels = as.character(c(0:19)))
```
